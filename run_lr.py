...
from utils.utils import top1_accuracy, grad_norm, \
    create_model_dir_lr, init_metrics_meter, extend_metrics_dict, metric_to_dict
...
# Main Modules
from worker_lr import MomentumWorker, MarinaWorker, DianaWorker,TopMomentumWorker,ByzantineWorker,EF21Worker,SIGNSGDWorker
from server import TorchServer
from simulator_lr import ParallelTrainer, DistributedEvaluator
...
def initialize_worker(
    ...
    attack_compression = get_compression("contractive_compressorlr")
    train_loader = libsvm(
    ...
    if args.attack == "LF":
        attacker =  LableFlippingWorker(
            server = server,
            revertible_label_transformer=lambda label: 1 - label,
            compression=attack_compression,
            data_loader=train_loader,
            model=model,
            model_snap=model_snap,
...
def main(args):
    ...
    # RandomNumber.full_grad_prob = 0.1

    if not args.dry_run:
        full_metrics = init_metrics_meter(metrics)
        model_dir = create_model_dir_lr(args)
